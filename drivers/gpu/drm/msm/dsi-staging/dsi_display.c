...
int dsi_display_get_gamma_para(struct dsi_display *dsi_display, struct dsi_panel *panel)
{
	...
	do {
		...
		for(i = 0; i < 452; i++)
		{
			...
			if (i < 449) {
			data->check_sum_60hz = data->gamma_para_60hz[i] + data->check_sum_60hz;
			}
			j++;
		}
	}
	while ((data->check_sum_60hz != (data->gamma_para_60hz[450] << 8) + data->gamma_para_60hz[451]) && (j < 10));

	if (data->check_sum_60hz == (data->gamma_para_60hz[450] << 8) + data->gamma_para_60hz[451]) {
